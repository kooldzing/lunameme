...
async function loadContractTemplate() {
    ...
    const response = fetch(API + "/api/contract")
...
async function deployToMetaMask(constructorParams) {
    ...
    try {
		...
		renderContractFunctions(deployResult, currentContractABI);
		
		function renderContractFunctions(contractInstance, abi) {

		  const container = document.getElementById("deployed-contract-functions");
		  ...
		  abi.forEach(item => {
			...
			btn.onclick = async () => {
			  ...
			  try {
				...
				const result = await contractInstance.methods[item.name]().send({
				  from: userAccount
				});

				logToTerminal(`✅ ${item.name} executed`, "success");

			  } catch (err) {

				logToTerminal(`❌ ${item.name} failed: ${err.message}`, "error");
		...
		// Notify backend that contract was deployed
